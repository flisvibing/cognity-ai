...
- **Uploaded model support:** can load `cognity_model/` with `cognity_tokenizer/` when those local folders are present
...
Cognity first tries to load the uploaded local Cognity transformer model folders if present, then falls back to the LSTM checkpoint. See `MODEL_FILES.md` for the expected folder layout.
...
## Use uploaded Cognity model files
...
`chat.py` will automatically prefer `cognity_model/` and `cognity_tokenizer/`. Legacy `tiny_gpt2_chatbot_model/` and `tokenizer_bpe/` folders still work for older drops. If they are absent, Cognity uses the character-level LSTM checkpoint from `cognity_0.1.pt`.
