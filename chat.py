...
2. the user's locally trained Cognity checkpoint stored in
   `cognity_model/` with its BPE tokenizer in `cognity_tokenizer/`.
...
COGNITY_MODEL_DIR = Path("cognity_model")
COGNITY_TOKENIZER_DIR = Path("cognity_tokenizer")
LEGACY_TINY_GPT2_MODEL_DIR = Path("tiny_gpt2_chatbot_model")
LEGACY_TOKENIZER_BPE_DIR = Path("tokenizer_bpe")
...
class CognityTransformerChat:
    """Local inference for the user's trained Cognity transformer checkpoint."""

    backend_name = "cognity-transformer-bpe"

    def __init__(
        self,
        model_dir: str | Path = COGNITY_MODEL_DIR,
        tokenizer_dir: str | Path = COGNITY_TOKENIZER_DIR,
    ) -> None:
    ...
    def _validate_files(self) -> None:
        ...
        if missing:
            raise FileNotFoundError("Missing Cognity transformer model files: " + ", ".join(missing))
    ...
    def _validate_dependencies(self) -> None:
        ...
        if missing:
            raise RuntimeError(
                "Install Cognity transformer runtime dependencies first: pip install -r requirements.txt. "
...
def _transformer_files_exist(model_dir: Path, tokenizer_dir: Path) -> bool:
    required = [
...
def _load_transformer_if_available() -> ChatBackend | None:
    candidates = [
        (COGNITY_MODEL_DIR, COGNITY_TOKENIZER_DIR),
        (LEGACY_TINY_GPT2_MODEL_DIR, LEGACY_TOKENIZER_BPE_DIR),
    ]
    for model_dir, tokenizer_dir in candidates:
        if _transformer_files_exist(model_dir, tokenizer_dir):
            try:
                return CognityTransformerChat(model_dir=model_dir, tokenizer_dir=tokenizer_dir)
            except RuntimeError as exc:
                print(f"Cognity transformer model found but could not be loaded: {exc}")
    return None


def load_or_stub() -> ChatBackend | None:
    """Prefer the uploaded Cognity transformer model, then fall back to LSTM."""
    transformer = _load_transformer_if_available()
...
if __name__ == "__main__":
    ...
    if bot is None:
        raise SystemExit(
            "No local model found. Add cognity_model/ + cognity_tokenizer/ "
